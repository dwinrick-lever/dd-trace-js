...
class PrioritySampler {
  ...
  configure (env, opts = {}) {
    ...
    log.info(`[PRIORITY SAMPLER CONFIG] Environment: ${env}`)
    log.info(`[PRIORITY SAMPLER CONFIG] Sample rate: ${sampleRate}`)
    log.info(`[PRIORITY SAMPLER CONFIG] Rate limit: ${rateLimit}`)
    log.info(`[PRIORITY SAMPLER CONFIG] Rules count: ${(rules || []).length}`)
    log.info(`[PRIORITY SAMPLER CONFIG] Rules: ${JSON.stringify(rules)}`)
    log.info(`[PRIORITY SAMPLER CONFIG] Provenance: ${provenance}`)

    this._env = env
    this._rules = this.#normalizeRules(rules || [], sampleRate, rateLimit, provenance)
    this._limiter = new RateLimiter(rateLimit)

    log.info(`[PRIORITY SAMPLER CONFIG] Final rules count after normalization: ${this._rules.length}`)
    this._rules.forEach((rule, i) => {
      log.info(`[PRIORITY SAMPLER CONFIG] Rule ${i}: sampleRate=${rule.sampleRate}, matchers=${rule.matchers.length}`)
    })
  ...
  sample (span, auto = true) {
    ...
    log.info(`[SAMPLING DEBUG ${spanResource}] Processing span: name=${spanName}, resource=${spanResource}, service=${context._tags['service.name']}`)

    const tag = this._getPriorityFromTags(context._tags, context)
    log.info(`[SAMPLING DEBUG ${spanResource}] Manual priority from tags: ${tag}, hasManualDrop: ${Object.hasOwn(context._tags, MANUAL_DROP)}, manualDropValue: ${context._tags[MANUAL_DROP]}`)

    if (this.validate(tag)) {
      context._sampling.priority = tag
      context._sampling.mechanism = SAMPLING_MECHANISM_MANUAL
      log.info(`[SAMPLING DEBUG ${spanResource}] Set manual priority: ${tag}, mechanism: ${SAMPLING_MECHANISM_MANUAL}`)
    } else if (auto) {
      context._sampling.priority = this._getPriorityFromAuto(root)
      log.info(`[SAMPLING DEBUG ${spanResource}] Set auto priority: ${context._sampling.priority}, mechanism: ${context._sampling.mechanism}`)
    } else {
      return
    }
  ...
  _getPriorityFromAuto (span) {
    ...
    const spanResource = context._tags.resource || context._tags['resource.name']
    log.info(`[SAMPLING DEBUG ${spanResource}] Finding rule for span. Found rule: ${rule ? 'YES' : 'NO'}`)
    if (rule) {
      log.info(`[SAMPLING DEBUG ${spanResource}] Rule details: sampleRate=${rule.sampleRate}, service=${rule.matchers.length > 0 ? 'with matchers' : 'no matchers'}`)
    }
  ...
  #getPriorityByRule (context, rule) {
    ...
    const spanResource = context._tags.resource || context._tags['resource.name']
    log.info(`[SAMPLING DEBUG ${spanResource}] Rule sampling: sampleRate=${rule.sampleRate}, ruleSample=${ruleSampleResult}, rateLimit=${rateLimitResult}, final=${finalResult}`)
  ...
  #normalizeRules (rules, sampleRate, rateLimit, provenance) {
    log.info(`[NORMALIZE RULES] Input: rules=${JSON.stringify(rules)}, sampleRate=${sampleRate}, rateLimit=${rateLimit}, provenance=${provenance}`)

    rules = Array.isArray(rules) ? rules.flat() : [rules]
    log.info(`[NORMALIZE RULES] After array normalization: ${JSON.stringify(rules)}`)

    rules.push({ sampleRate, maxPerSecond: rateLimit, provenance })
    log.info(`[NORMALIZE RULES] After adding default rule: ${JSON.stringify(rules)}`)

    const result = []
    for (const rule of rules) {
      const parsedSampleRate = Number.parseFloat(rule.sampleRate)
      log.info(`[NORMALIZE RULES] Processing rule: ${JSON.stringify(rule)}, parsedSampleRate=${parsedSampleRate}, isValid=${!Number.isNaN(parsedSampleRate)}`)
      ...
      if (!Number.isNaN(parsedSampleRate)) {
        ...
        log.info(`[NORMALIZE RULES] Created rule with sampleRate=${normalizedRule.sampleRate}, matchers=${normalizedRule.matchers.length}`)
        result.push(normalizedRule)
      } else {
        log.info(`[NORMALIZE RULES] Skipped invalid rule: ${JSON.stringify(rule)}`)
      }
    }
    log.info(`[NORMALIZE RULES] Final result: ${result.length} rules`)
    return result
  }
