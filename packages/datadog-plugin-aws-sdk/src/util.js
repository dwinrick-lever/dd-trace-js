...
function encodeValue (valueObject) {
  ...
  try {
    ...
    switch (type) {
      ...
      default:
        log.info(`Found unknown type while trying to create DynamoDB span pointer: ${type}`)
    }
  } catch (err) {
    log.info(`Failed to encode value while trying to create DynamoDB span pointer: ${err.message}`)
  }
}
